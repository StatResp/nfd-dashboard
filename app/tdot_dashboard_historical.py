...
def init_dashboard(server):
    ...
    #TODO - added dot in front of read pickle
    print("READING DIRECTORY")
    print(os.getcwd())
    seg = pd.read_pickle('app/data/tdot/grouped_inrix.pkl')
    seg = seg[seg.frc == 0]
    ...
    metadata = {}
    metadata[
        'merged_pickle_address'] = 'app/data/tdot/merged_4h_2017-04-01_to_2021-06-01_top20percent_segments_grouped_nona'
    metadata['incident_pickle_address'] = 'app/data/tdot/incident'
    metadata['pred_name_TF'] = 'incident_occurred'
    available_features = ['is_weekend', 'window',
                          'speed_mean', 'average_speed_mean', 'reference_speed_mean', 'congestion_mean', 'miles',
                          'lanes',
                          'isf_length', 'slope_median', 'ends_ele_diff', 'temp_mean', 'wind_spd_mean', 'vis_mean',
                          'precip_mean',
                          'mean_incidents_last_7_days', 'mean_incidents_last_4_weeks',
                          'mean_incidents_over_all_windows',
                          ]
    ...
    tncounties = pd.read_csv('app/counties.csv')
    ...
    dash_app = dash.Dash(__name__,
                         ...
                         server=server,
                         #Dash will control everything beneatch prefix
                         routes_pathname_prefix="/dashapp/",
                         meta_tags=[
            {"name": "viewport", "content": "width=device-width"}])
    dash_app.title = 'Incident Dashboard'


    # TODO - NOT SURE WHERE TO PUT THIS STUFF IN BETWEEN ###############################
    server.secret_key = os.environ.get('secret_key', str(randint(0, 1000000)))
    cache = Cache(dash_app.server,
    ...
    # Layout of Dash App
    dash_app.layout = html.Div(id='container-div', className="container-fluid bg-white text-dark", children=[html.Div(
        children=[
            html.Div(
                className="row",
                children=[
    ...
    init_callbacks(dash_app, cache, df, df_merged, startdate, enddate,tncounties)

    return dash_app.server


def init_callbacks(app, cache, df, df_merged, startdate, enddate, tncounties):
    # TODO - NOT SURE WHERE TO PUT THIS STUFF IN BETWEEN ###############################
    ...
    def update_map_incident_predictions(counties, darktheme, timechosen):
        ...
        today = date.today()
        df = get_prediction_frame(today.strftime("%d/%m/%Y"))
        latone = Config.LATINITIAL
        lonone = Config.LONINITAL
        zoomvalue = 9
        countycondition = True
        daycondition = True
        timecondition = True
...
# MXM - add to allow authorization on dash prefix
...
# %%
#MXM - Remove because running own Flask server
