...
df_merged = pd.read_parquet(metadata['merged_pickle_address'], columns=available_features+['time_local','county','incident_occurred'])
...
df = df.rename(columns={'gps_coordinate_longitude':'longitude','gps_coordinate_latitude':'latitude','incident_id':'incidentNumber'})
...
def return_incidents(start_date, end_date, counties, months, timerange,   days):
    ...
    if True:
        ...
        result = df[timecondition & date_condition & month_condition & weekday_condition & county_condition]
...
def incidentsfeature(df_merged, feature):
    ...
    df_merged_ = categorize_numerical_features(df_merged[[feature,metadata['pred_name_TF']]])
    ...
    df_merged_cat_barplot = df_merged_[[metadata['pred_name_TF'], feature_cat]].groupby(feature_cat).agg('count').rename(columns={metadata['pred_name_TF']:'freq'}).reset_index()
    ...
    df_incidentcount=df_merged_[[metadata['pred_name_TF'],feature_cat ]].groupby(feature_cat).agg(['mean','sum'])
    ...
    fig4 = px.bar(df_incidentcount, x=feature_cat, y='mean',category_orders=category_orders)
    #fig4.write_html('hist_test4.html')

    #putting all figures in one
    fig = make_subplots(rows=4, cols=1, subplot_titles=("Histogram of the Data", "Histogram of the categorized Data", "Total number of incidents in each category", "Average number of incidents in each category"))
...
def incidentsfeaturecombo(result, datemonth):
    ...
    result = categorize_numerical_features(result[['incident_occurred','congestion_mean','temp_mean','precip_mean','is_weekend']])
    result = FILTER_calculator(result[['incident_occurred'] + list(All_Possible_Filters.keys())], All_Possible_Dic, 'All')
    result=result.rename(columns={'Average_Number_Incidents':'Average Number Accidents'})
    # drawing
    fig = px.bar(result, x='Tag', y='Average Number Accidents')
    layout = go.Layout(
        bargap=0.05,
        autosize=True,
        bargroupgap=0,
...
def update_bar_chart(start_date, end_date, counties, datemonth, histogramkind, timerange,   days, feature):
    ...
    if histogramkind == "incidentsfeature" or histogramkind == "incidentsfeaturecombo":
        result = return_merged(start_date, end_date, counties, datemonth, timerange, days)
    else:
        result = return_incidents(start_date, end_date, counties, datemonth, timerange,   days)
